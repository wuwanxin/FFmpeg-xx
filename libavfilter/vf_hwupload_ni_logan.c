...
static av_cold int niupload_init(AVFilterContext *ctx)
{
    ...
    av_dict_set(&s->opts, "devname", s->device_name, 0);

    return av_hwdevice_ctx_create(&s->hwdevice, AV_HWDEVICE_TYPE_NI_LOGAN, buf, s->opts, 0);
}
...
static const AVOption niupload_options[] = {
    ...
    { "devname", "Name of the device to use", OFFSET(device_name), AV_OPT_TYPE_STRING, { .str = "" }, CHAR_MIN, CHAR_MAX, FLAGS },
    { NULL },
};
