...
#include "version.h"
// include startcode.h starting from FFmpeg n5.1.2
...
{
  int ret;
  ni_logan_decoder_params_t *p_param = &s->api_param;

  /* Synchronize parameters between XCoderH264DecContext, ni_logan_session_context_t,
     and ni_logan_decoder_params_t */

  // 1. hwframes can be set from 'out=hw' or 'hwframes 1' param
  ...
  // 2. overwrite set_high_priority value here with a custom value if it was provided
  if(s->api_param.dec_input_params.set_high_priority != 0)
  {
    s->api_ctx.set_high_priority = s->api_param.dec_input_params.set_high_priority;
  }
  else
  {
    s->api_ctx.set_high_priority = s->set_high_priority;
  }
  av_log(avctx, AV_LOG_TRACE, "Custom NVMe set_high_priority set to = %d\n",
         s->api_ctx.set_high_priority);

  // 3. overwrite keep alive timeout value here with a custom value if it was provided
  // if xcoder option is set then overwrite the (legacy) decoder option
  if (s->api_param.dec_input_params.keep_alive_timeout != NI_LOGAN_DEFAULT_KEEP_ALIVE_TIMEOUT) 
  {
    s->api_ctx.keep_alive_timeout = s->api_param.dec_input_params.keep_alive_timeout;
  } 
  else 
  {
  ...
  // 4. overwrite UDU passthru value here with a custom value if it was provided
  if (s->api_param.dec_input_params.enable_user_data_sei_passthru != 0)
  {
    s->enable_user_data_sei_passthru = s->api_param.dec_input_params.enable_user_data_sei_passthru;
  }
  else
  {
    s->api_param.dec_input_params.enable_user_data_sei_passthru = s->enable_user_data_sei_passthru;
  }

  // 5. overwrite check_packet value here with a custom value if it was provided
  if (s->api_param.dec_input_params.check_packet != 0)
  {
    s->enable_check_packet = s->api_param.dec_input_params.check_packet;
  }
  else
  {
    s->api_param.dec_input_params.check_packet = s->enable_check_packet;
  }

  // 6. overwrite UDU passthru value here with a custom value if it was provided
  if (s->api_param.dec_input_params.custom_sei_passthru != NI_LOGAN_INVALID_SEI_TYPE)
  {
    s->custom_sei = s->api_param.dec_input_params.custom_sei_passthru;
  }
  else
  {
    s->api_param.dec_input_params.custom_sei_passthru = s->custom_sei;
  }

  // 7. overwrite lowdelay value here with a custom value if it was provided
  if(s->api_param.dec_input_params.lowdelay != 0)
  {
    s->low_delay = s->api_param.dec_input_params.lowdelay;
  }
  else
  {
    s->api_param.dec_input_params.lowdelay = s->low_delay;
  }
  ...
  s->api_ctx.hw_name = s->dev_dec_name;
...
int ff_xcoder_logan_dec_close(AVCodecContext *avctx, XCoderLoganDecContext *s)
{
  ...
  ni_logan_decoder_params_t *p_param = &s->api_param; //dec params in union with enc params struct
  int suspended = 0;
                             ...
                             AVPacket *pkt)
{
  ...
  if (xpkt->data_len == 0)
  {
    ...
    struct AVBSFContext *bsf = avctx->internal->bsf;
#else
    ...
    if (s->custom_sei != NI_LOGAN_INVALID_SEI_TYPE)
    {
      ret = ni_logan_detect_custom_sei(pkt->data, pkt->size, &s->api_ctx, &s->api_param, xpkt);
      if (ret != 0)
      {
        goto fail;
      }
...
#if ((LIBAVCODEC_VERSION_MAJOR > 60) || (LIBAVCODEC_VERSION_MAJOR == 60 && LIBAVCODEC_VERSION_MINOR >= 31))
    if (s->draining && !s->eos) {
      av_log(avctx, AV_LOG_ERROR, "%s draining, recv == 0, eos 0!\n", __func__);
      return AVERROR_EXTERNAL;
    }
#endif
    if (s->eos) {
...
#if ((LIBAVCODEC_VERSION_MAJOR > 60) || (LIBAVCODEC_VERSION_MAJOR == 60 && LIBAVCODEC_VERSION_MINOR >= 31))
      if (s->draining) {
          // FFmpeg-n6.1 doesn't allow egain at draining stage
          goto read_op;
      }
#endif
      return AVERROR(EAGAIN);
