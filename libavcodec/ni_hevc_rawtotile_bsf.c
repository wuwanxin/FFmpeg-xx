...
{
    CodedBitstreamH265Context *priv = s->cbc->priv_data;
    uint8_t force_tile = 2;
    ni_write_nal_header(&s->stream, HEVC_NAL_PPS, 0, 1);
    if ((s->column * s->row) <= 1)
        force_tile = 0;
    return ni_hevc_encode_nal_pps(&s->stream, priv->active_pps, force_tile, s->column, s->row);
}
...
AVBitStreamFilter
#endif
ff_hevc_rawtotile_bsf = {
#if (LIBAVCODEC_VERSION_MAJOR > 59 || LIBAVCODEC_VERSION_MAJOR >= 59 && LIBAVCODEC_VERSION_MINOR >= 37)
        .p.name         = "hevc_rawtotile",
        .p.codec_ids    = hevc_rawtotile_codec_ids,
        .p.priv_class   = &hevc_rawtotile_class,
#else
        .name           = "hevc_rawtotile",
