...
int xcoder_decode_close(AVCodecContext *avctx)
{
  ...
  for (int i = 0; i < NI_FIFO_SZ; i++) {
    if (s->gs_data[i].buf0 != NULL) {
      av_buffer_unref(&s->gs_data[i].buf0);
    }
  }
#endif
    ...
    case AV_CODEC_ID_HEVC:
      ...
      switch (avctx->profile)
      {
        ...
        case NETINT_SKIP_PROFILE:
          av_log(avctx, AV_LOG_ERROR, "Warning: HEVC profile %d not supported, skip it\n", avctx->profile);
          break;
        default:
    ...
    default:
      ...
      switch (avctx->profile)
      {
        ...
        case FF_PROFILE_UNKNOWN:
          break;
        case NETINT_SKIP_PROFILE:
          av_log(avctx, AV_LOG_ERROR, "Warning: H264 profile %d not supported, skip it.\n", avctx->profile);
          break;
        default:
          av_log(avctx, AV_LOG_ERROR, "Error: profile %d not supported.\n", avctx->profile);
          return AVERROR_INVALIDDATA;
