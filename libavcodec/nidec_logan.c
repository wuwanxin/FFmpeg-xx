...
int xcoder_logan_decode_init(AVCodecContext *avctx)
{
  ...
  {
    av_log(avctx, AV_LOG_DEBUG, "Warning: profile unknown\n");
  }
  ...
  // Initialize ni_session_context
  if ((ret = xcoder_logan_setup_decoder(avctx)) < 0)
  {
    return ret;
  }

  // Initialize xcoder context
  s->started = 0;
  s->offset = 0LL;
  s->draining = 0;
  s->got_first_key_frame = 0;
                                     ...
                                     AVFrame *frame, bool wait)
{
  ...
  if (s->buffered_pkt.size > 0 &&
      s->api_ctx.ready_to_flush == NI_LOGAN_DECODE_FLUSH_BUFFER_OFF)
  {
    ...
    {
      av_log(avctx, AV_LOG_ERROR, "ff_xcoder_logan_dec_send() return error:%d\n", ret);
      av_packet_unref(&s->buffered_pkt);
      return ret;
    }
    else
...
{
  XCoderLoganDecContext *s = avctx->priv_data;
  av_log(avctx, AV_LOG_DEBUG, "%s enter\n",__func__);
  if (s->api_ctx.ready_to_flush == NI_LOGAN_DECODE_FLUSH_BUFFER_OFF)
      ni_logan_decoder_session_send_flush(&(s->api_ctx));
}
