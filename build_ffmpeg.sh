...
enable_ni_measure_latency=false;
...
while [ "$1" != "" ]; do
    case $1 in
        -h | --help) echo "Usage: ./build_ffmpeg.sh --quadra | --logan [OPTION]";
                     ...
                     echo "--latency                     compile with Netint filter latency measurement.";
        ...
        ;;
        --latency)       enable_ni_measure_latency=true
        ;;
...
if [[ $target_windows == "false" && $target_android == "false" && $enable_ni_drawtext == "true" ]]; then
    extra_config_flags="${extra_config_flags} --enable-libfreetype --enable-libfontconfig --enable-pic"
else
    product_line_flags="${product_line_flags} --disable-filter=drawtext_ni_quadra --enable-pic"
fi

if $debug; then
    extra_config_flags="${extra_config_flags} --disable-optimizations --disable-asm --disable-stripping --enable-debug=3"
else
    extra_config_flags="${extra_config_flags} --disable-debug"
fi
...
if $enable_ni_measure_latency; then
    extra_config_flags="${extra_config_flags} --extra-cflags=-DNI_MEASURE_LATENCY"
else
    extra_config_flags="${extra_config_flags} --extra-cflags=-UNI_MEASURE_LATENCY"
fi
