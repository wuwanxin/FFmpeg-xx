...
{ //#include "ni_util.h"
    ...
    // Add enough space for roi and other meta data
    extra_len = (frame->width * frame->height / (8 * 8)) * 2;
    extra_len = ((extra_len + 63) & (~63));     // make it align to 64 bytes
    frame->buf[0] = av_buffer_alloc_quadra(ret + extra_len);
